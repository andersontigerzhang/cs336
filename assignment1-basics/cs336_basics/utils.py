...
def init_regex():
    global RE
    if RE is None:
        RE = re.compile(GPT2_PRETOKENIZER_PATTERN)

def text_to_pretoks(special_tokens, text, include_special_tokens=False):
    global RE
    pattern = '|'.join(re.escape(token) for token in sorted(special_tokens, key=len, reverse=True))
    spans = re.split(f'({pattern})', text)
    tokens = []
    for span in spans:
        if span in special_tokens:
            if include_special_tokens:
                tokens.append((span.encode("utf-8"),))
        else:
            for match in RE.finditer(span):
